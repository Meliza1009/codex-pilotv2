...
(async () => {
  ...
  await withPrEnv({ GITHUB_PR_TOKEN: 'tok', CODEX_PILOT_PR_MODE: 'fork' }, async () => {
    ...
    const cloneCall = git.calls.find((c) => c.args.includes('clone'));
    assert.ok(cloneCall && cloneCall.args.join(' ').includes('https://github.com/merinelizabeth/repo.git'), 'clones the existing fork');
    assert.ok(events.some((e) => e.type === 'completed'), 'emits completed');
  });
  ...
  await withPrEnv({ GITHUB_PR_TOKEN: 'tok', CODEX_PILOT_PR_MODE: 'fork' }, async () => {
    const fetchImpl = mockFetch([
      ['GET https://api.github.com/user', { status: 200, body: { login: 'merinelizabeth' } }],
      ['GET https://api.github.com/repos/merinelizabeth/repo', { status: 404, body: { message: 'Not Found' } }],
